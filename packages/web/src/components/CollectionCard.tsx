...
export function CollectionCard({
  ...
  return (
    <button
      className={["atlas-coll", done ? "atlas-coll--done" : "", className]
        .filter(Boolean)
        .join(" ")}
      onClick={onClick}
    >
      ...
      <span className="atlas-coll__body">
        <span className="atlas-coll__top">
          <span style={{ minWidth: 0 }}>
            ...
            <span className="atlas-coll__title" style={{ display: "block" }}>
              {title}
            </span>
          </span>
          <span className="atlas-coll__count">
            <b>{value}</b> / {max}
          </span>
        </span>
        ...
        <span className="atlas-coll__pct">
          {done
            ? "Completed"
            : `${Math.round(pct)}% complete · ${max - value} remaining`}
        </span>
      </span>
    </button>
  );
