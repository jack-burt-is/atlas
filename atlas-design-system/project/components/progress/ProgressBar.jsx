...
export function ProgressBar({
  ...
  const display = valueFormat
    ...
    : max === 100 ? `${Math.round(pct)}%` : `${value} / ${max}`;
  return (
    <div className={['atlas-pbar', `atlas-pbar--${size}`, className].filter(Boolean).join(' ')} {...rest}>
      {(label || showValue) && (
      ...
      <div className="atlas-pbar__track">
        <div className={`atlas-pbar__fill atlas-pbar__fill--${fillColor}`} style={{ width: `${pct}%` }} />
      </div>
    </div>
  );
}
