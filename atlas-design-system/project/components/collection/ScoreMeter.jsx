...
export function ScoreMeter({
  score = 0,
  level = 1,
  levelProgress = 0,   // 0..100 toward next level
  toNext = null,       // points remaining label
  className = '',
  ...rest
}) {
  ensure();
  ...
  return (
    <div className={['atlas-score', className].filter(Boolean).join(' ')} {...rest}>
      ...
      <div className="atlas-score__body">
        ...
        <div className="atlas-score__track"><div className="atlas-score__fill" style={{ width: `${pct}%` }} /></div>
        <span className="atlas-score__next">{toNext != null ? toNext : `${Math.round(pct)}% to level ${level + 1}`}</span>
      </div>
    </div>
  );
}
